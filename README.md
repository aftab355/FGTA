...
#### K — how far one result is allowed to move a rating

K is the step size of the Elo update, and it is flat: every game moves both players by the same K (`K = 32`), scaled equally for both sides by the margin-of-victory multiplier. `matchKPair()` is the one place this lives — it always returns the same number for both sides of a match, which is what keeps the rating pool strictly zero-sum: it always sums to exactly `players x START`, checked directly in the Lab's data audit and on the ladder's own pool line.

The app previously ran a dynamic, per-player K (bigger for newcomers, smaller for settled players, boosted after a layoff) that let the two sides of a game move by different amounts, and let the rating pool drift away from `players x START` by a tracked, accounted-for amount. It was deliberately reverted to a flat K, by request: a newcomer now moves at the same rate as a fifty-game veteran, and #1 can be unseated by a single upset just as easily as any other rank — trading the dynamic version's faster-comeback-for-the-bottom, harder-to-shake-#1 shape for a simpler, always zero-sum ladder.
...
Displaying it was never the problem: the scorecard prints the numbers it is given, per set, and a two-digit column reads fine. **Summing it was.** Only one of those three deciders lies about its own size. `3-1` is small and reads small. `10-7` has the shape of a set won ten games to seven and is the *shortest* thing anybody plays, so every total taken over the sets column read it about ten times too big:

- **Longest match** in the record book went to whichever match had a breaker in it. `6-4, 4-6, 10-8` summed to 38 games, which beat a genuine `7-6, 6-7, 7-5` marathon — while being half an hour shorter.
- **Margin of victory** read a `10-2` finish as a ten-games-to-two set, which out-dominated a double bagel. A breaker win was earning a bigger K multiplier than beating the same player 6-0 6-0.
- **Tiebreaks** counted only sets that finished 7-6, so a match decided *in a tiebreak* didn't count as one — on a profile, in the league totals, or in the pre-match briefing line.
- **Score reconstruction** (Matches → Auto-cut) expanded `10-7` into a seventeen-game set of serve-and-ends rotation. That does not merely fail; it fails quietly and takes the two real sets with it, because every run after the first is then off by a set's worth of rotation.

What identifies a breaker is that **it is not a legal set score**. Nobody wins a set with more than seven games — 6-0 through 6-4, 7-5, 7-6, and that is the whole list — so a pair above that ceiling can only be a breaker written into the sets column. `SET_MAX_GAMES`, `setPairs()`, `setOversized()` and `breakerIndex()` in the Elo engine are the one reading of that, and there are deliberately two questions:

- `setOversized(pair)` — *is this pair too big to be a set at all?* The right question for "how much tennis was this", which is what `kitMatchMinutes()` asks. A group game played to 11 is short too, and should read short.
- `breakerIndex(pairs)` — *which pair, if any, is a **decider** played as a breaker?* The last pair of a multi-set score, above the ceiling. Only that shape gets a games count invented for it, so a lone `11-8` group game is never mistaken for one.

With that, a breaker decider is counted as **the one decisive game it stood in for** in the margin multiplier (`6-2, 4-6, 10-7` now scores within 0.05x of the same match won `6-2, 4-6, 6-4`), as **two games** in Longest match, and as **a tiebreak** wherever tiebreaks are counted. A first-to-3 decider needs none of this — `4-2` is inside the ceiling, sums small, and was always read correctly.

**The margin change is date-gated** (`MOV_BREAKER_START`, 2026-09-19), for the same reason `MOV_START` and `BLOWOUT_FLOOR_START` are: the table is recomputed from the match rows on every load, so changing how a *score* is read moves published ratings unless the change starts on a day. Breaker matches already in the archive keep the rating they earned.

**Auto-cut sets the breaker aside rather than guessing at it.** `scBreakerAt()` spots it in the typed final score; `scScore()` reconstructs the sets before it, reports the breaker as the set it stood in for with no games under it, and says so in the panel — which is what [docs/scoring.md](docs/scoring.md) always promised ("tiebreaks are reported, not reconstructed"), now actually true of the score that says `10-7` as well as the one that says `7-6`. Its rallies are left out of the games list and out of the burnt-in board rather than filled with invented rotation.
...
- Requires two tables. **Paste [`docs/kit.min.sql`](docs/kit.min.sql)** into the Supabase SQL editor; read [`docs/kit.sql`](docs/kit.sql) for why any of it is shaped the way it is. Both optional — without them the screen names *which* table is missing and points at the file, because "we never created that table" and "you have not added a racket yet" look identical from the outside and want completely different actions from the reader.
  - **Paste it on its own**, for the reason the site-config migration learned the hard way: Supabase runs an editor tab as one transaction, so one error anywhere — including a harmless "already exists" from an unrelated script above it — rolls back everything after it, and you are left with no tables and an error message about something else. `kit.min.sql` assumes nothing else exists and every statement is guarded, so running it twice is safe.
...
- **Twenty-two text/background pairs below WCAG AA**, found by walking every element of all eleven views and measuring rendered colour against its nearest opaque ancestor. Four causes, all systematic rather than one-off:
  - `--dim` and `--ball` are text colours, and both had been set from their ratio against **white alone**. They cleared AA on `--bg` (4.82 and 4.61) and quietly missed it on the two tinted grounds they land on just as often — `--panel-2` (4.26 / 4.07) and `--accent-dim` (4.08 / 3.90). A "dim" colour is by definition the one closest to failing, so it is the one that has to be checked against the *darkest* ground it appears on, not the lightest. Now 5.80 / 5.12 / 4.91 and 5.77 / 5.10 / 4.88.
  - `renderTickerBar()` wrote `color:var(--accent)` **inline** on the scrolling text, over the bar's `background:var(--chrome)`. Championship green on chrome green: **1.69:1**, on the one strip visible from every screen. The bar already sets `color:var(--on-chrome)` for exactly this reason, and a comment two rules down says `.tick-item` has to read off the chrome rather than the page — the inline style beat both.
  - The go-live button was white on `#ff2d2d`, 3.71:1. Darkened to 5.2:1; the live dot beside it keeps the bright red, because a dot is a signal, not something anybody reads.
...
**Runs Aug 17 – Sep 13.** Those two dates live in one place: the `FF CUP WINDOW` script just after `</header>` in `index.html`. The badge in the top bar, the lockup on the load screen and the marquee on the landing page all read from them (the script publishes `window.FFCUP` for the app), so they can't drift. To move the cup, change `START` / `END` there and nothing else.
...
**It retires itself.** After Sep 13 the window script removes `data-skin` on load and the app is back to normal with no intervention — it removes *its own* skin only, which is a distinction that did not exist while ffcup was the only skin and started mattering the day the Fall Exhibition skin below arrived. That leaves only the dead CSS to delete:
...
- **The marquee.** For the length of the cup the event is the first thing on the page, above the broadcast hero on the ladder and above the timeline on Home. One component (`renderCupMarquee()`, the `FF CUP — THE MARQUEE` block) fills every `.cup-mount` stub on the page: a jet-black slab with a neon filament welded along its top edge, a court grid receding into the floor behind the type, one specular crossing the face every nine seconds, the wordmark struck in chrome, and a live countdown burning through it as exposed tube — to first serve before the cup, to the last day once it's running. Under it, the six-stage rail (Roster → Group → Cut → Semis → Final → Champion) reads live off the Robin+ snapshot, then **the bracket itself** (see below), and the footer carries the next fixture and the way in. Nine views get it — the two landing pages and Events in full, every other top-level tab as a compact lit bar; Messages is left alone. Nothing is stored twice: the marquee reads the real tournament row for its field, stage, next fixture and champion, and only falls back to `window.FFCUP`'s dates while that row doesn't exist yet.
- **The bracket, promoted.** Saying what stage the event is at and who plays next is two thirds of a promo: it never showed the *shape* of the thing. So the marquee carries the whole draw as a board — four seeds at the corners, a road in from each to its semifinal, one road out of each semifinal into the cup in the middle — drawn off the same `rpSnapshot` the event page draws, so the two can't disagree. The roads are the point: each is three SVG strokes (kerb, surface, and the paint down the middle), the paint on a road somebody is travelling **flows towards the cup**, and a tennis ball runs the road of any tie being played right now. A seed that has gone through lights up, one that has gone out is struck through and faded, and when it is over only the champion's route is still lit. Before the group is settled there is no bracket to draw, so it draws the one the table would make **if the group ended tonight** and says `PROJECTED` — which is the more promotable picture anyway: everyone can see who they'd get. The geometry lives on plain classes and the skin re-dresses it in neon (black tarmac, magenta kerbs, cream paint) without a second copy; container units scale the type with the picture, so a 300px card and a 560px one draw the same drawing. Under `prefers-reduced-motion` the paint stops flowing and no ball is emitted.
...
- **The walk-in.** The title film a fixture gets on the day it is played (`FFIntro`, the `FF CUP — THE BRACKET RUN` block). One shot, eleven seconds, and **the event's own bracket is the set**: a lit interchange hanging in the dark with the cup at the middle of it, four seed roads running in from the corners, two merging at each semifinal, one road out of each semifinal into the ring the final is run on, and a web of group-stage roads feeding the whole thing from out in the fog. The camera opens a long way above it, where the four arms and the cup read as one drawing, then falls out of the sky onto the tarmac and stays there. The ball comes in off the group and takes **this fixture's own road** — its player's seed, its seed road, its semifinal, the road out of it to the final — crosses the ring and goes into the cup. Whiteout, and a **versus card**: both players, their rating, their record in this cup, their last five, the head-to-head, what the winner walks into (`Winner meets X in the final`), and the Elo split — the last of which the run has spent ten seconds working out in front of you.

  *(It used to be a flight over a procedural city, with the bracket as a three-second prologue. There is now one film and it is the bracket one: the city, its skyline generator, the window the camera flew through and the arena it landed at are gone.)*

  **Everything standing by the road is real.** The seeds are the seeds, the signs at the junctions carry the actual pairings, the route is the route to the actual final — all read from `film.bracket`, which is the same `rpSnapshot` the marquee's board is drawn from, so the film and the card cannot disagree. A road nobody is on is unlit tarmac; the road this fixture's player came in on is the bright one; a road already travelled is gold; and once the cup is won only the champion's route is still lit.

  **The ball's road is the route itself**, sampled off the same spline and laid flat — not an arc drawn beside it. That is the difference between a ball on a road and a ball that drifts off one, and it drifted. The road is laid in lengths rather than as one polyline, because the painter's sort places a road by its middle and the middle of a road eleven seconds long is behind the camera for most of the film.

  Scattered along it is the app itself, the way it used to be scattered through a skyline: the brand ball hanging where the moon should be (seam and all), the **rating galaxy** as a constellation on the dome, the ladder's **podium** with its three medals and a crowd beside the ring, the top bar's **ticker** — `LIVE · K 20-64 · START 500`, read straight off `taglineText()` so the film cannot drift from the bar — running along a wall by the seed road, the load screen's **court** painted on the road to the final, a floodlit **show court** mid-rally off the seed road, a **trophy** on a plinth out in the group, the **FGTA wordmark** in neon with one tube not quite right, jumbotrons cycling the fixture and the cup, camera drones, a blimp, birds breaking off as the ball passes, and `FX.celebrate`'s confetti drifting through all of it.

  Every one of those was authored for the old avenue, which means every one of them builds its quads facing `+z`. Rather than teach two dozen draw functions about headings, **the camera is turned instead**: `brkSpin(C, phi)` is the same camera seen from a world rotated about the y axis — rotate its position the other way, add `phi` to its yaw, leave pitch and roll alone — and `brkFrame` puts a world position into that same frame. A sign planted beside a road at any angle squares up to it, for about fifteen lines of new code.

  **The sum, out loud.** The card lands on a split — `68% · 32%` — and a number with nothing behind it is a horoscope, so the film does the working on the way there. A strip in the top corner steps through the ladder's own Elo expectation, one line at a time across the run: the two live ratings, the gap between them, the gap over the divisor, ten to that power, and finally `E = 1 / (1 + 10^(−Δ/400))` resolving a beat before the cut — so the odds bar on the card is the **answer to a question the viewer watched being asked**. The board carries the same sum twice: a lit board beside the seed road with the two ratings on it, and one slung over the road to the final with the formula on it. Every figure comes from `mathOf()`, derived once from the film's own `divisor` and `K`, so the strip, the boards and the line under the odds bar cannot drift apart — and the bar itself is now read off that working rather than computed beside it. Films restored from the offline cache predate those two fields and fall back to classic Elo.

  **It says whose film it is.** An `FGTA` mark — brand ball, wordmark, `championship ladder` — sits in the corner for the whole run and stays up through the card, over a corner scrim so it survives a lit sign passing behind it. The card signs itself with the same lockup above the fixture line, and labels its odds bar `FGTA Elo · expected score` with the formula under it. In the world, the boards are headed `FGTA · ELO`. Under the opening shot, one line names what you are looking at — the two semifinals, or the champion once there is one — and it is gone by the time the camera is on the road, which is when the fixture's own strap takes over.

  It is one `<canvas>` and one overlay, drawn from nothing: no library, no assets, hand-rolled perspective projection with a painter's sort. The scenery is scattered from the fixture's own key, so every game gets its own arrangement and the same game always gets the same one. Measured on a phone-sized frame: median 3ms a frame, worst 7ms — about a third of the budget, and cheaper than the city it replaced.

  **Every move has a reason.** The camera rides the road a beat behind the ball — a *time* offset along the same spline, not a step backwards in space, because on a road that turns this hard a step backwards puts the camera in the dark beside it looking at a ball with nothing under it. The height is the only other thing that moves: three hundred metres up for the shot that shows the whole bracket, down on the tarmac by three seconds, and up again a little for the ring, because from six feet the last turn is a wall and from twenty it is a turn. It looks at the cup at the two ends and at the ball in the middle, and the ball's lead shortens over the last two seconds so that a hero it has been following for nine of them doesn't leave the frame at the finish. The bank comes off the turn the road is in, and nothing else is authored at all.

  It plays from the ▶ button on any fixture with two named players, from the marquee's **Watch the walk-in** on the day, when the cup is opened on a day that has a game, and **when the app itself is opened** on such a day. All routes go through the same once-per-game-per-device guard, so it is an occasion rather than a toll gate. Tap, Esc or **Skip** ends it; under `prefers-reduced-motion` the run is skipped entirely and the card comes up on its own.

  **On a match day it is instant, and it holds the screen.** Waiting for the database before deciding whether to play would put the film some hundreds of milliseconds *after* the load screen, which reads as a stutter rather than an opening. So the decision is made with no network at all, from two synchronous sources tried in order: `fgta_walkin_next` (a finished film, written at the end of every `load()` for the next dated fixture the event is waiting on) and, failing that, the app's existing `fgta_offline_cache`, from which the film is re-derived. Either way `FFIntro.prepare()` builds the board and paints frame zero *while the load screen is still up*, and the run starts on the exact frame the wipe begins — `ffWhenRevealed`, published by the load screen itself, with `.loadscreen` at `z-index:10000` so its wipe **reveals** a run already going rather than cutting to one. Measured: the overlay is built and painted ~2.3s before the wipe, so there is nothing left to compute at the cut.
  ...
  **What keeps it smooth.** The route is a Catmull-Rom spline through eighteen keyframes rather than a set of hand-written phases — the first version of the old flight switched between phases and the camera's *speed* jumped where two met, a visible hitch in the middle of the best shot; a spline is continuous in velocity everywhere, so there is no seam left to hit. Yaw is not authored at all: it is taken from where the camera is looking, and the bank from the turn the road is in. Beyond that: the versus card is built and held paused rather than injected at the cut (parsing it mid-run cost the frame it landed on), every bloom radius is capped at 42% of the frame (a lamp passing a metre from the lens was asking for a 700px radial gradient — one call painting more than the whole canvas), the backing store is capped well below the display's pixel ratio because nothing at a hundred units a second is worth four times the fill, and a lane marking closer than twenty metres is skipped, because at that range it is a white slab across the frame rather than a road.
...
A second, parallel leaderboard laid over the same match log — **not** a ladder reset. From `FALL_START` (Sept 14) every ladder game counts twice: once for the real ladder, exactly as before, and once toward this — ranked not by rating but by how much of it a player has gained since the moment the fall window opened.

**The baseline is never stored.** `computeFallExhibition()` (in the `FALL-EXHIBITION` region of `index.html`, right after the Elo engine it reuses) replays the approved, Elo-counting match log chronologically through the same `matchKPair`/`DIVISOR`/`START` the ladder uses. The first time a player is seen at or after `FALL_START`, whatever their rating is at that instant — carried forward from their last pre-fall game, or `START` if they have none — *is* their baseline, because nothing moves a rating except a game and there is none between then and now. A correction to an old match reflows into the baseline exactly the way it reflows into the ladder; the two can't disagree, and there's nothing to go stale.

**Eligibility floor:** a player needs `FALL_MIN_GAMES` (3) Elo-counting games inside the window to be ranked. Fewer than that and they sit in a separate "not yet eligible" list showing their count — they're never silently dropped, and a player who hasn't played a single fall game doesn't appear at all (there is nothing to rank them on).

**The end date is admin-editable** — the one thing here with real state — via the ✎ controls under the Events-tab card, backed by the single-row `fall_exhibition` table (**docs/fall-exhibition.sql**; the app runs fine without it, just pinned to the coded default). Matches after that date don't exist to the replay at all, which is what "finalizes" the event: the ladder keeps moving, the fall numbers stop.

**Events tab:** a promo card ahead of the tournament list, reusing the app's existing "upcoming event" card language — `starts Sept 14` → `N days left` → `final standings`, opening into the ranked table.

This section used to end by saying the exhibition would deliberately *not* get a skin, because "two promotable events sharing one skin-attribute system would collide". That was a true statement about two events running at once. These two don't: the FF Cup closed on Sep 13 and the exhibition opens on Sep 14, so `data-skin` is only ever asked to carry one of them. The skin below is what took its place.

Covered by `test/fall-exhibition.test.js`: the baseline against a plain pre-fall replay, a brand-new player's baseline landing on `START`, the eligibility floor and its sort order, the end-date cutoff, and that a non-counting match is as invisible here as it is to the ladder.

### The skin — "late light"

For the run of the exhibition the app wears a fall skin. It is the opposite decision to the FF Cup's: the cup graded the page into a dark, blown-out frame, and this one keeps the championship system exactly as it is — light paper, dark type, one accent that marks everything you can act on — and moves only the *temperature*. A ladder table is something people read for ten minutes at a time, and a season is not a reason to make that harder.

**How it is switched on:** `data-skin="fall"` on `<html>`, same contract as the cup. The CSS is one block in the `<style>` in `index.html`, headed `FALL EXHIBITION SKIN`, and every rule in it is scoped to `:root[data-skin="fall"]` — take the attribute off and the site is the championship theme again with nothing else changed. The Admin Studio's **Skin** control writes the same attribute.

**What it does:**

- **The palette, one season warmer.** Paper goes to cream (`--bg #FDF6EC`, `--panel #FFFBF5`, `--panel-2 #F3E3CE`), type to bark (`--ink #2B1708`), the accent from championship green to burnt maple (`#A33A12`), the chrome from dark green to roasted brown (`#3A2010`). The signals move with it: with an orange accent, `--danger` would sit too close to it, so it is pulled toward wine (`#94202B`) and `--win` toward harvest olive (`#4C6B1B`) so a result still reads as a result. The two counterweights in the depth layer are deliberately *not* warm — pine `#1D5138` and lake `#1F6070` — because that is what stops a page of orange flattening into one hue.
- **Measured against its own paper.** Every token ever used as type was checked against all three of the skin's grounds rather than against white, because the cream `--panel-2` is the darkest paper in the skin and is therefore the one that decides. The text golds had to come down a step for exactly that reason (the default `#8A6A1E` lands at 4.39:1 on cream and quietly misses AA — the same trap the base palette documents against white). Worst-case ratios: `--ink` 13.58, `--muted` 7.95, `--dim` 5.48, `--accent` 5.26, `--court` 5.39, `--gold` 5.14, `--ball` 4.98, `--win` 4.87, `--danger` 6.67. The floor is 4.87:1; `--surface-bevel-light` and `--glow-mesh` are the two exemptions the base palette already claims, since one is a 1px edge and the other a backdrop wash.
- **A fall chart series.** Eight hues that walk the wheel (rust, ochre, olive, pine, lake, indigo, aster, wine) rather than staying inside the season's warms, since eight shades of orange are eight series nobody can tell apart. Same rule as the default set — printing weight, separated by hue rather than lightness — measured on cream: 4.87:1 at worst.
- **The opening.** The load screen is maple leaves opening onto the wordmark: eleven leaves gathered into a bud at the centre of a dark frame, fanning outward along their own angles, turning as they go and drifting off while `FGTA LADDER` comes up through the gap they leave, with the season named under it. The loader is the one dark surface in the skin and that is deliberate — the app underneath is cream, which is the right ground for a table and the wrong one for a maple leaf (burnt orange on near-white is a smudge), so the opening is played at dusk under the canopy and the wipe at 2.4s opens onto the daylight page.
- **The drift.** Eight leaves coming down behind the whole document at 12–16% opacity on 18–37s cycles. Not per-panel — eight elements for the page, which is the lesson from the hot weather theme's deleted flames (three infinite animations on every panel is not a look, it is a cost). It has its own switch in the Studio's FX list ("Falling leaves"), separate from the load screen, because a 2.4s cinematic once a session and a loop that runs as long as the tab is open are not the same decision.
- **Weather still speaks, quietly.** The skin's palette outranks the live weather themes, so `body[data-heat]` / `body[data-wx]` can no longer retint the app while it is on — by design; a season skin that changes colour with the forecast is not a skin. What survives is the *texture*: the rain drip on panels keeps running, in bark, because "the covers are on" is information rather than decoration.
- **The ball stays a tennis ball.** `.tb-ball` is painted from literals and is left alone. A tennis ball is fluorescent yellow-green in every season; recolouring it would be costume, not theme.

**Two bugs it surfaced, both the same shape.** Each was a piece of code that had only ever seen one skin and had quietly written that assumption down as a fact:

1. **The FF Cup window script removed `data-skin` unconditionally** once the cup was over. From Sep 14 that ran on every load and took the fall skin off before the first paint — the attribute was in the markup and looked like it was doing nothing. It now removes only `ffcup`.
2. **`applySurface()` put the palette straight back.** The ☰ menu's court toggle writes `--accent` / `--accent-dim` / `--bg` as *inline* custom properties on `<body>`, which beat every rule in the stylesheet; the cup skin already opted out of it by name. So the maple accent was live on `<html>` and on every rule in the sheet, and three inline properties painted championship green back over the buttons, the links and the report form on a page that was otherwise entirely autumn. The opt-out is a table now (`SKIN_OWNS_PALETTE`) rather than a string comparison — a skin that redefines the palette belongs in it, and the court toggle keeps working under one that doesn't.

**Cleaning up after it.** The same three steps as the cup:

1. Take `data-skin="fall"` off `<html>` (or set the Studio's Skin control to "Off"), and put the `theme-color` meta back to `#0B3B26` and `manifest.webmanifest`'s `theme_color` / `background_color` back to `#0B3B26` / `#FFFFFF`.

   The manifest is worth a note of its own: it was still carrying the FF Cup's `#000000` for both, which step 3 of the cup's own cleanup above asked for and nobody did — so an installed app was opening on a black splash and then painting a white page. It is the fall chrome and the *loader's* ground now (`#3A2010` / `#1D0F05`), which makes the PWA splash and the leaf opening one continuous dark frame rather than two.
2. Delete the `FALL EXHIBITION SKIN` block from the `<style>`, the `<div class="ls-maple">` and `<div class="ls-season">` on the load screen, the `<div class="fall-leaves">` in the body, the `fall` entries in `CHART_SERIES` and `SKIN_OWNS_PALETTE`, the `fallleaves` FX feature and the `fall` option in the Studio's Skin control.
3. The palette itself does not have to go with it: it is also in the Studio's preset list as **Fall exhibition**, which is where to click if the colours should outlive the event.
...
- supabase/functions/recap/ — the one Supabase Edge Function: the weekly round-up, written server-side on a cron. Not a duplicate of `/api/ai` — that one is a proxy the browser calls, this one runs when nobody has the app open and reads the database directly. `facts.js` computes the fact sheet in code and the model never sees a match row; see **docs/edge-functions.md**.
- docs/edge-functions.md — deploying `recap`, its secrets (including why `RECAP_SECRET` is not optional), the `pg_cron` schedule, and why it cannot post twice.
...
- docs/fall-exhibition.sql — the one-row `fall_exhibition` table holding the admin-editable end date for the Fall Exhibition leaderboard. The baseline and standings need no migration at all — they're replayed live off `matches`; see **Fall Exhibition** below.
...
- `elo.test.js` — **the rating engine**, which had no coverage at all until now, and is the one piece of this app whose output people argue about. A bug in the feed loses a post and somebody notices within the hour; a bug in here quietly rewrites who is winning and produces a table exactly as plausible as the right one. Covers the margin multiplier, the per-player K, the replay, and — most importantly — **the date gates**. Margin of victory and the dynamic K were both shipped with an explicit promise not to re-score anything already played, and that promise is enforced entirely by two string comparisons; the last section replays a pre-gate season and checks it against a flat-K replay written out by hand, to the point. The newest section is the one that reads a decider played as a breaker: that `10-7` is not a set score, that a lone `11-8` group game is not a decider, and the comparison that used to come out backwards — squeaking a 10-2 breaker measured *more* dominant (1.377x) than bagelling a real third set (1.354x).
- `fall-exhibition.test.js` — the Fall Exhibition leaderboard's own replay. Checks the baseline against an independent plain replay rather than hand-derived numbers, a brand-new player's baseline landing on plain `START`, the 3-game eligibility floor and which list a player falls into on either side of it, that a game after the admin end date moves neither the gain nor the fall-game count, that a non-Elo-counting game is as invisible here as it is to the ladder, and that a player with zero fall games doesn't appear at all rather than showing as a padded zero.
...
- `site-config-sql.test.js` — every `docs/*.min.sql` against the `docs/*.sql` it was generated from, statement for statement, plus the guards that keep each migration safe to re-run (guarded tables and indexes, a `drop policy` for every `create policy`, nothing dropping a table). Two copies of a migration is a trap — somebody fixes a policy in one and ships the other for a year — so the duplication is only worth it if drift is impossible to miss.
