...
- `setOversized(pair)` — *is this pair too big to be a set at all?* The right question for "how much tennis was this", which is what `formatReliability()` and `kitMatchMinutes()` ask. A group game played to 11 is short too, and should read short.
...
With that, a breaker decider is counted as **the one decisive game it stood in for** in the margin multiplier (`6-2, 4-6, 10-7` now scores within 0.05x of the same match won `6-2, 4-6, 6-4`), as **two games** in Longest match, as **a tiebreak** wherever tiebreaks are counted, and as **`DYNK_TB_WORTH` = 6 games** in the format reliability that scales K. A first-to-3 decider needs none of this — `4-2` is inside the ceiling, sums small, and was always read correctly.
