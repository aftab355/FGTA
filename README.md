...
- **Showtime** (see its own section below): an optional FX layer over the whole app — a particle canvas, card tilt, podium foil, decoding headings, a scroll ribbon and an idle screensaver. Off under `prefers-reduced-motion`, and every visitor can turn it down or off.
...
## Showtime — the FX layer

Section 12 of the stylesheet and the last `<script>` in the body. It is decoration and nothing else: no function in it changes a rating, a match, a view or a stored value, and deleting both blocks leaves an app that behaves identically and looks quieter. Everything below is *how it stays that way*, because a decoration that can break the app is not worth having.

**What it does.** A particle canvas (pointer trail, press shockwaves, a confetti cannon, fireworks, ambient drift, ball rain), a 3D tilt on small cards, a metal foil sweep on the podium three, headings that decode themselves the first time they scroll into view, a scroll progress ribbon with a ball on the leading edge, an idle screensaver where a ball rallies off the real boxes on the page, and four typed easter eggs (`ace`, `boom`, `roll`, `rain`) plus the Konami code for **Ludicrous**. The app's own celebrations — `confetti()`, `celebrateResult()`, `FX.celebrate()` — are wrapped rather than edited, so they keep being the source of truth for *when* a celebration happens and this only decides what it looks like.

**Four levels**, set from ✨ Showtime in the overflow menu and stored per visitor in `localStorage.fgta_showtime`: Off, Calm, Showtime (the default) and Ludicrous. Per-effect switches sit under them, along with a live frame-time readout — the honest answer to "is this costing me anything" is a measured number and not a reassurance.

**The escape hatch: `?fx=off`.** Every other switch is inside the app — the ✨ Showtime panel is reached from the overflow menu, the admin flag from the Studio — and both assume the page is usable enough to tap. The one failure this layer could ever cause is a page that is not, so there is a way in from the address bar that needs nothing on screen: `fgta.netlify.app/?fx=off` turns it off *and stores that*, so the next load is clean too. `?fx=on` puts it back. It is read before anything else in the module runs, so a URL that says off means no canvas is ever created, no listener attached and no class added — not "created and then hidden". It is also the fastest way to answer "is the FX layer doing this": if `?fx=off` looks the same, it is not.

**Three vetoes, any one sufficient.** `prefers-reduced-motion: reduce`, the admin's `showtime` feature flag in the Studio, and the visitor's own Off. The OS setting outranks the other two, which is the right order: somebody who has told their phone they get motion sick did not come here to be asked again. The admin flag is *read* by the engine rather than *applied* to the visitor — for exactly the reason the sound flag is, one entry above it in `FEATURES`: writing "off" into each visitor's localStorage would overwrite their own setting and leave it overwritten after the admin switched the layer back on.

**Why it holds 60fps on a phone.** One `requestAnimationFrame`, one canvas, four listeners, all passive. The particle pool is a structure of arrays allocated once and never grown, so a celebration allocates nothing and cannot trigger a collection pause in the middle of itself. Every soft particle is one pre-rendered radial gradient blitted with `drawImage` — there is not one per-particle gradient or one `shadowBlur` in the draw loop. Four things then keep it honest under load:

- **A frame-time governor.** The loop grades itself on an EMA of its own frame time and takes the particle budget away from itself when frames run late, giving it back slowly. On an older Android that means fewer sparks; it does not mean a dropped frame.
- **A fill-rate ceiling, which matters more than the particle cap.** Cost is pixels, not particles: one clear plus one composite of a 2880×1800 backing store is 5.2M pixels a frame before anything is drawn, and that number does not care how many sparks there are. The backing store is capped in absolute pixels per device tier and the DPR solved backwards out of it, so a phone keeps its sharpness and a laptop quietly runs its canvas at ~1.3.
- **A dirty rect.** The clear is the box the last pass actually drew into, not the viewport. The common case by a long way is a thumb-sized pointer trail in one corner; the confetti cannon reports a screen-sized box and pays what it always paid.
- **It parks.** When nothing is alive the loop cancels itself, and it refuses to run at all while the tab is hidden.

Measured in a headless (software-rasterised, no GPU) Chromium, which is a worst case: on a 390×844 phone profile, 60fps idle with the layer on against 60 with it off, 59 during a full celebration, 36 in Ludicrous. The second canvas this started with cost about a third of the page's frame rate for an ambient layer that was, behind a white page, almost invisible — so there is one canvas now and the motes are drawn in front at low alpha, where they read as dust on the lens and are better anyway.

**Two things it learned from the page it sits on.**
- *Additive blending is a dark-theme technique.* On the white page this app actually is, `lighter` turns sparks into muddy grey-green smears — white plus anything is still white. So the ground's luminance is measured from `--bg` and the particles switch: opaque ink drawn source-over on a light page, additive glow on a dark one. Both cases ship (the page is white, the FF Cup skin and the night weather themes are not), so this is a switch that genuinely flips rather than a hedge.
- *Rows do not tilt.* The first version tilted the podium rows and it was wrong for a reason that predates it — the `.pod` tilt above is documented as deliberately not applied to full-width panels or list rows, from direct user feedback, because a wide short rectangle leaning toward a pointer reads as a table coming loose rather than as depth. Small, roughly square things tilt. The podium three get the foil instead, which is an edge treatment and moves nothing.

**Where it composes rather than clobbers.** The tilt and the lift are written with the individual `rotate:`/`translate:` properties, never `transform`, because those apply *before* `transform` in the cascade — so `.panel:hover{transform:translateY(-3px)}` and the `panelIn`/`modalPop` entrances keep working underneath. There is deliberately **no scroll-reveal**: the app already has one (`panelIn` on panels, `rowIn` on all twenty-eight row classes, both staggered), and two entrance animations on one element is a fight neither wins.

**What it puts back.** The press ripple needs `position:relative` and `overflow:hidden` on whatever was pressed, and restores both — a stray `overflow:hidden` left on an app's buttons is how a decoration quietly breaks a dropdown three views away, months later, in a way nobody connects to a ripple. The decode writes the heading's text node in place and always restores the exact original string, so a re-render is not something it has to survive. `perspective` is only ever put on a container the file recognises *and* that holds nothing `position:fixed`, because an element with a perspective becomes the containing block for fixed descendants — put one on the wrong container and a modal stops being pinned to the viewport.
...
- The Showtime layer lives in index.html with everything else: section 12 of the `<style>` block and the last `<script>` in the body. Both are self-contained and both are deletable — see **Showtime — the FX layer** above.
...
- `showtime.test.js` — the FX layer, in a real browser, and almost every assertion is something it must **not** do: not swallow a tap through its full-viewport canvas, not overflow a 390px phone, not leave inline styles on the app's buttons after a ripple, not leave a heading scrambled, not emit a single particle once it is off or under `prefers-reduced-motion`, and not write a visitor's stored preference when an *admin* drops the site-wide flag. Needs a Playwright chromium; says so and exits 2 rather than failing.
