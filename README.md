...
- **The walk-in.** The title film a fixture gets on the day it is played (`FFIntro`, the `FF CUP — THE WALK-IN` block). The camera leaves the road, climbs over a floodlit rooftop court mid-rally, dives back through the plaza, banks out of the avenue and goes **straight through a lit office window** — corridor, strip lights, poster on the wall, out the far side — rolling through a full 360° on the way, then up over the skyline, back down among the towers, and out to the arena floating above the city. Whiteout, and a **versus card**: both players, their rating, their record in this cup, their last five, the head-to-head and the Elo split.
  ...
  **On a match day it is instant.** Waiting for the database before deciding whether to play would put the film some hundreds of milliseconds *after* the load screen, which reads as a stutter rather than an opening. So the app leaves itself a note — `fgta_walkin_next`, written at the end of every `load()` for the next dated fixture the event is still waiting on. On the next boot that note is read synchronously before any network at all: if it is for today and unwatched, `FFIntro.prepare()` generates the city and paints frame zero *while the load screen is still up*, and the flight starts on the exact frame the wipe begins (`ffWhenRevealed`, published by the load screen itself; the load screen sits at `z-index:10000` so its wipe reveals the film underneath rather than cutting to it). The cost is that the card's numbers are as of the last session — at worst one session stale, which is the right trade. The first boot after a schedule is made has no note yet, and `ffBootWalkIn` covers that one case off the back of `load()`.
