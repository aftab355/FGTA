...
- **Content**: Standings are computed live from the matches table (Elo, K=32, start rating 500).
...
- **Point Tracker**: Enter player 1 / player 2 names → "Start tracking" begins a live point-by-point 1v1. Best-of-3, games to 6 win-by-2, tiebreak to 7 at 6-6. Plain incrementing point counts (0,1,2,3…), not tennis scoring. Every tap is undo/redo-able; nothing writes to the ladder until final Submit. Every tap is also **timestamped**, which is the whole basis of the rally reel described under Archive below — the ref pressing a button when a point ends is a more reliable record of where the rally finished than tracking the ball would be, and it was already happening.
  ...
  - **Submit toward**: a tracked game files to the ladder by default, but a "Submit toward" picker on both the setup screen and the wrap-up lets the ref file it under any unfinished event instead — the FF Cup leads the list while it's on. Picking an event sets `matches.tournament_id`, the same field the admin tournament screens write, so the event page and the draw pick the game up with nothing new downstream. Whether the game *also* moves Elo is `tournamentCountsElo()` rather than a second switch, so the two can never disagree: an exhibition files the result and leaves ratings alone, while the FF Cup always moves them. When the two players are an unplayed fixture in that event's draw, the round is attached too and the result slots straight into the bracket on approval. The choice is remembered between matches (a ref working a cup reffs the whole cup) and frozen onto the match at start, so resuming a draft can't re-file it under a target picked for a later game.
...
## The FF Cup counts for Elo

The cup is the season's tournament, and it moves the ladder. `tournamentCountsElo(t)` — the single gate the engine, the marquee badge and the ref's "Submit toward" note all read — returns `true` for it regardless of the `counts_elo` stored on its row, because the Robin+ format it runs under sets that flag to `false` when the row is made. Without the exception the ladder during the cup was a record of everything *except* the cup: a player could win it outright and end the month exactly where they started.

The cup is identified by **name** (`CUP_NAME_RX`), never by `cupTournament()`. That helper falls back to "the newest unfinished event" so the marquee can promote the cup before the draw exists — the right answer for promotion, and the wrong one here, where it would quietly drag every other exhibition onto the ladder with it. Every other event still answers with its own flag.

Because standings are recomputed from approved history on every load, turning this on re-rates **every cup game already played**, not just the ones from here on. To take it back off, delete the `isCupEvent` line from `tournamentCountsElo()`.
...
4. **Before deleting the marquee block, set `counts_elo = true` on the cup's row in the database.** The Elo exception is keyed on `CUP_NAME_RX`, which lives in that block; with it gone `isCupEvent()` catches the reference error and returns `false`, and because standings are recomputed from approved history on every load, the cup's games would silently stop counting and every rating would move. Writing the flag onto the row makes the exception redundant, and the cup keeps the ratings it earned.
