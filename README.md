...
**It retires itself.** After Sep 13 the window script removes `data-skin` on load and the app is back to normal with no intervention. That leaves only the dead CSS to delete:
...
**Events tab:** a promo card ahead of the tournament list, reusing the app's existing "upcoming event" card language rather than a bespoke skin — `starts Sept 14` → `N days left` → `final standings`, opening into the ranked table. Deliberately lighter than the FF Cup's treatment below: two promotable events sharing one skin-attribute system would collide, and a leaderboard doesn't need a cinematic.
