...
- **The calendar — a day for every game.** Any Robin+ fixture can be given a day inside the cup window: group games, qualification deciders, the semis, the third-place game and the final, whether or not the round it belongs to has been reached yet (the semis are schedulable before anyone has qualified for them). An admin sets a day from a date picker on the fixture itself or from the calendar panel, or lays the whole draw out at once with **Spread across the window**, which distributes the fixtures evenly in playing order across the days between the cup's start and end. The panel shows the window as a strip of day cells — a dot per fixture, filled once it's played — then the running order underneath with today's game called out. The days live in `state.dates` inside the same `tournaments.bracket` JSON as the draw, keyed by round, so **there is no second migration**; they also survive a re-draw, because a day is a slot in the window ("the third game is on the 21st") rather than a property of a pairing. Off-skin, and before a draw exists, the panel renders nothing.
- **The walk-in.** The title film a fixture gets on the day it is played (`FFIntro`, the `FF CUP — THE WALK-IN` block): a flight down a night avenue past what the city is doing — a floodlit rooftop court mid-rally with the ball on a real arc, a plaza of phone lights facing a screen, a jumbotron running the fixture, banners strung over the street, an elevated tram, a drone swarm, the blimp, a terrace still full, fireworks over the far rooftops — out over the last buildings, up to the arena floating above the city, then a whiteout into a **versus card**: both players, their rating, their record in this cup, their last five, the head-to-head and the Elo split. It is one `<canvas>` and one overlay, drawn from nothing: no library, no assets, and the city is generated from the fixture's round key, so every game gets its own skyline and the same game always gets the same one. Hand-rolled perspective projection with a painter's sort; buildings drop their window grids by distance and the whole generation is scaled back on phones and low-core devices. It plays from the ▶ button on any fixture with two named players, from the marquee's **Watch the walk-in** on the day, and once per game per device when the cup is opened on a day that has one. Tap, Esc or **Skip** ends it; under `prefers-reduced-motion` the flight is skipped entirely and the card comes up on its own.
