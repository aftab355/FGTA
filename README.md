...
- **The walk-in.** The title film a fixture gets on the day it is played (`FFIntro`, the `FF CUP — THE WALK-IN` block). It opens on the event's own shape — see **the bracket run** below — and comes out of that into the city: the camera leaves the road, climbs over a floodlit rooftop court mid-rally, dives back through the plaza, banks out of the avenue and goes **straight through a lit office window** — corridor, strip lights, poster on the wall, out the far side — rolling through a full 360° on the way, then up over the skyline, back down among the towers, and out to the arena floating above the city. Whiteout, and a **versus card**: both players, their rating, their record in this cup, their last five, the head-to-head, what the winner walks into (`Winner meets X in the final`), and the Elo split — the last of which the flight has spent ten seconds working out in front of you.

  **The bracket run — the first three seconds.** The film used to open on a street. It now opens on the bracket, at speed: a lit interchange hanging in the dark with the cup at the middle of it, four seed roads running in from the corners, two merging at each semifinal, and one road out of each semifinal into the ring around the cup. The camera starts a long way above the board — far enough that the four arms and the cup read as one drawing — and drops out of the sky onto the tarmac inside the first second and a half. The ball comes in off the group stage, takes **this fixture's own road** (its player's seed, its semifinal, its road to the final), and goes into the cup; the light that makes is the cut to the city. Everything standing by the road is real: the seeds are the seeds, the semifinal signs carry the actual pairings, and the route is the route to the actual final, all read from `film.bracket` off the same snapshot the marquee's board uses. Same engine as the flight — same projection, same fog, same helpers — in its own world with the cup at the origin. The film's clock (`T_BRK`, then `T_CUT`) and the flight's own clock are kept separate, so every beat written in flight seconds still means what it did.
