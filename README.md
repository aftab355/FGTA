...
- **The cut.** Ranked on **Pts** → head-to-head → game difference → games won → strength of opposition → ladder rating. A group game can be drawn and, mid-group, the rows have not played the same number of games — so neither wins nor point totals order the table honestly. `Pts` is the **share of the points on offer a player has actually taken**: a win is 1, a draw is ½, over the games they have played. Ranking on wins made a draw worth exactly what a loss is worth; ranking on the total then ranked whoever had played most, which put an unbeaten 2-0-1 in three games below a 3-1 in four. Once every row has played the same number of games the share orders them exactly as the totals do, so the final table is unaffected — this only decides what the table says while the group is still running. A row with no games yet has no share to report and sits below anyone who has started. The record column reads **W-L-D** and always prints all three numbers, so a draw can never read as a loss (or hide behind a column the row above it doesn't have); on a narrow phone the last tiebreak column drops rather than the table overflowing. Anything results can settle *is* settled on results; **anything still level across the 4th/5th line plays a 1v1 decider for the spot** rather than being broken on a tiebreak column. `rpPlayoffPlan()` picks the decider shape from the arithmetic: 2 level for 1 spot → one game; 5 level for 4 → the bottom two play; 3 level for 1 → a mini knockout with a bye to the better seed; and (vanishingly rare) 5+ level for 2 → the same ring format run among the tied.
